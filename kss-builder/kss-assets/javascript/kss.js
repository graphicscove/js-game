...
(function() {
  ...
  (function smoothScrolling () {
    $('.kss-nav__menu-item > a[href*=section]').on('click', function (e) {
  ...
  (function() {
    $('.kss-toggle-nav').on('click', function() {
        $('.kss-nav__menu').slideToggle();
    })
  })();

  (function() {
    $('.kss-nav__name').each(function() {
        var item = $(this);
  ...
  (function() {
    $('.kss-section-header').each(function() {
        var item = $(this);
        var text = item.text().split('.')[1]
        item.html(text)
    })
  ...
  (function(){
    ...
    if (parameters) {
      $('.kss-parameters__item').each(function (index) {
        ...
        if (isColor) {
          $(this).parent().addClass('kss-colors-container').prev().hide();
